<div class="card">
    ...
    <div class="body">
        ...
        <div class="row clearfix">
            <div class="col-md-12">
                <form action="<?= base_url() ?>create_lajur" method="POST">
                    <input type="hidden" name="id_surat" value="<?php echo $surat['surat_id'] ?>">


                    <div class="form-group">
                        <label>Keterangan</label>
                        <textarea rows="4" class="form-control" name="lajur_keterangan"
                                  placeholder="Masukkan keterangan"></textarea>
                    </div>
                    <div class="form-group">
                        <label for="">Tujuan Disposisi</label>
                        <select name="lajur_jabatan" class="form-control">
                            <option disabled selected>Pilih Tujuan Disposisi</option>
                            <?php
                            $jabatan = $this->JabatanModel->getJabatanBagian();
                            ?>
                            <?php foreach ($jabatan as $value): ?>
                                <option value="<?= $value['jabatan_id'] ?>"><?= $value['jabatan_nama'] ?>
                                    - <?= $value['jabatan_bagian'] ?></option>
                            <?php endforeach ?>
                        </select>
                    </div>
